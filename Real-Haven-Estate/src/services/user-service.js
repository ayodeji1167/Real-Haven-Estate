...
class UserService {
  ...
  registerUser = async (req) => {
    ...
    const confirmAccountToken = newUser.createConfirmEmailToken();
    const confirmAccountUrl = `${req.protocol}://${req.get(
      'host',
    )}/api/v1/user/confirmaccount?token=${confirmAccountToken}`;
    const confirmAccountMessage = `Please make a put request to: ${confirmAccountUrl} to confirm your account`
    await newUser.save()
  ...
  confirmRegisteredEmail = async (req) => {
    ...
    let userExist = await UserModel.findOne({
      comfirmEmailToken:token,
      comfirmEmailTokenExpire:{ $gt: Date.now() }
    })
    ...
    userExist =  await UserModel.findOneAndUpdate({
      comfirmEmailToken:token,
      comfirmEmailTokenExpire:{ $gt: Date.now() }
    }, { confirmedAccount: true }, {new:true})
   

    userExist.comfirmEmailToken = undefined
    userExist.comfirmEmailTokenExpire = undefined
